...
const GLTFModel: FC<{ url: string; onLoaded?: () => void }> = ({ url, onLoaded }) => {
    const gltfContent = useGLTF(url);
    const content = gltfContent.scene.clone();
...
// FBX Model Component
const FBXModel: FC<{ url: string; onLoaded?: () => void }> = ({ url, onLoaded }) => {
    const fbxContent = useFBX(url);
    const content = fbxContent.clone();
...
// OBJ Model Component
const OBJModel: FC<{ url: string; onLoaded?: () => void }> = ({ url, onLoaded }) => {
    const objContent = useLoader(OBJLoader, url);
    const content = objContent.clone();
...
const ModelInner: FC<ModelInnerProps> = ({
    ...
    }, [modelLoaded, autoFrame, camera, fadeIn, initPitch, initYaw, onLoaded, pivot]);
    ...
    return (
        <group ref={outer}>
            <group ref={inner}>
                {ext === 'glb' || ext === 'gltf' ? (
                    <GLTFModel url={url} onLoaded={handleModelLoaded} />
                ) : ext === 'fbx' ? (
                    <FBXModel url={url} onLoaded={handleModelLoaded} />
                ) : ext === 'obj' ? (
                    <OBJModel url={url} onLoaded={handleModelLoaded} />
                ) : (
                    <Html center>
                        <div className="text-red-500">Unsupported format: {ext}</div>
                    </Html>
                )}
            </group>
        </group>
